...
class Ticket(Base):
    ...
    placements: Mapped[list["BetPlacement"]] = relationship("BetPlacement", back_populates="ticket", cascade="all, delete-orphan")
    ledger_entries: Mapped[list["LedgerEntry"]] = relationship("LedgerEntry", back_populates="ticket", cascade="all, delete-orphan")
...
class BetPlacement(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    ticket_id: Mapped[int] = mapped_column(ForeignKey("tickets.id", ondelete="CASCADE"), nullable=False)
    bookmaker_account_id: Mapped[int | None] = mapped_column(ForeignKey("bookmaker_accounts.id", ondelete="SET NULL"), nullable=True)
    ...
    settlement: Mapped["Settlement | None"] = relationship("Settlement", back_populates="placement", uselist=False, cascade="all, delete-orphan")
    ledger_entries: Mapped[list["LedgerEntry"]] = relationship("LedgerEntry", back_populates="placement", cascade="all, delete-orphan")
...
class Settlement(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    bet_placement_id: Mapped[int | None] = mapped_column(ForeignKey("bet_placements.id", ondelete="CASCADE"), nullable=True)
